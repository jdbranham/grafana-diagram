...
export const CompositeMetricEditor: React.FC<StandardEditorProps<CompositeMetric[], any, DiagramPanelControllerProps>> = (
    ...
    const memberOptions = (currentValue: string) => context.data?.map(m => {
        return (<option selected={m.name == currentValue} value={m.name}>{m.name}</option>);
    })
        ...
        return (
            <div>
                ...
                <div className="gf-form">
                    ...
                    <div className="gf-form">
                        <input value={composite.name} onChange={(ev) => handleCompositeNameChange(ev, composite)} type="text" className="gf-form-input width-15"></input>
                    </div>

                </div>
                <div className="gf-form">
                    <Checkbox css="" value={composite.showLowestValue} onChange={(ev) => handleShowLowestValueChange(ev, composite)} label='Show lowest metric'></Checkbox>
                </div>
                <div>
                    {memberList(composite)}
                </div>
                <div className="gf-form">
