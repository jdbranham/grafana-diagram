...
const selectElementByEdgeLabel = (container: HTMLElement, id: string): Selection<any, any, any, any> => {
    return select(container).selectAll('span')
        .filter(function () {
...
const selectDivElementByAlias = (container: HTMLElement, alias: string): Selection<any, any, any, any> => {
    const targetElement = select(container).selectAll('div')
        .filter(function () {
...
const selectTextElementByAlias = (container: HTMLElement, alias: string): Selection<any, any, any, any> => {
    return select(container).selectAll('text')
        .filter(function () {
            return select(this).text() == alias;
        });
};
...
const resizeGrouping = (element: Selection<any, any, any, any> | null | undefined, nodeSize: NodeSizeOptions) => {
    ...
    if(closestGroup && closestLabelGroup && closestForeignObject) {
        ...
        const _minWidth = Math.max(Number.parseInt(closestForeignObject.getAttribute('width') || '1', 10), nodeSize.minWidth);
        const _minHeight = Math.max(Number.parseInt(closestForeignObject.getAttribute('height') || '1', 10), nodeSize.minHeight);
...
const styleD3Shapes = (targetElement: Selection<any, any, any, any>, indicator: MetricIndicator, useBackground: boolean, nodeSize: NodeSizeOptions) => {
...
const styleFlowChartEdgeLabel = (targetElement: Selection<any, any, any, any>, indicator: MetricIndicator, useBackground: boolean, nodeSize: NodeSizeOptions) => {
...
const styleTextEdgeLabel = (targetElement: Selection<any, any, any, any>, indicator: MetricIndicator, useBackground: boolean) => {
...
const reduceModels = (models: DiagramSeriesModel[]): MetricIndicator[] => {
     return models.filter(m => m.valueField.config.custom).flatMap(m => {
...
export const updateDiagramStyle = (el: HTMLElement, models: DiagramSeriesModel[], options: DiagramOptions, diagramId: string) => {
